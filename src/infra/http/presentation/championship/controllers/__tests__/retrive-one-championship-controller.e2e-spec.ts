...
describe('RetriveOneChampionship end to end tests', () => {
  ...
  test('I expect a championship returns', async () => {
    const response = await request(app).get(`/championship/${championship.id}`)
  ...
  test('I expect the request to be rejected when the id does not exist', async () => {
    const response = await request(app).get(`/championship/fake-id`)
