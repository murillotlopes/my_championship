...
describe('RetriveOneTeam end to end tests', () => {
  ...
  test('I expect a team returns', async () => {
    const response = await request(app).get(`/team/${team.id}`)
  ...
  test('I expect the request to be rejected when the id does not exist', async () => {
    const response = await request(app).get(`/team/fake-id`)
